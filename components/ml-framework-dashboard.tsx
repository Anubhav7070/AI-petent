...
export function MLFrameworkDashboard() {
  ...
  const startTraining = () => {
      ...
      setModels((prev) =>
        prev.map((model) => {
          if (model.progress < 100) {
            ...
            const newAccuracy = (newProgress / 100) * (0.75 + Math.random() * 0.2)
            return {
              ...model,
    ...
    setTimeout(() => {
      ...
      setIsTraining(false)
      console.log("[v0] ML model training completed")
    }, 8000)
  }
          ...
          <Card key={model.name} className="relative overflow-hidden">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                <Badge variant={model.type === "classical" ? "default" : "secondary"}>
                  {model.type === "classical" ? "Classical ML" : "Deep Learning"}
                </Badge>
              </div>
            </CardHeader>
            <CardContent>
